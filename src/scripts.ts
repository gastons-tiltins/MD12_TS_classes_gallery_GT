...
class ImagesGallery {
  ...
  slidesLogic(n: number) {
    ...
    let circles: HTMLCollectionOf<Element> =
    ...
    (slides[this.sIndex - 1] as HTMLElement).style.display = 'block';
  }
}
...
class FullScreen extends Thumbnails {
  ...
  fullScreen = (i: number) => {
    let modal = document.getElementById('full-screen') as HTMLElement;
    let img = document.getElementById(`car${i}`) as HTMLImageElement | null;
    let modalImg: any = document.getElementById('f-image') as HTMLImageElement;
